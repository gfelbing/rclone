...
func (c *writerAt) WriteAt(buffer []byte, offset int64) (n int, err error) {
	...
	leaf, directoryID, err := c.fs.dirCache.FindPath(c.ctx, c.remote, true)
	if err != nil {
	...
	openResult, err := fileOpenNew(c.ctx, client, c.fs, directoryID, leaf)
	if err != nil {
	...
	// get target hash
	outChecksum, err := fileChecksum(c.ctx, client, c.fs.pacer, fd, offset, int64(contentLength))
	if err != nil {
		return 0, err
	}
	...
	if _, err := filePWrite(c.ctx, client, c.fs.pacer, fd, offset, buffer); err != nil {
		return 0, err
	}

	// close fd
	if _, err := fileClose(c.ctx, client, c.fs.pacer, fd); err != nil {
